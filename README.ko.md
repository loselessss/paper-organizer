...
AI가 필요 없으면 AI 설정에서 `서지정보와 초록만 입력 (AI 사용 안 함)`을 선택하세요.
모델·API 키 없이 PDF에서 읽은 서지정보와 초록만 저장하고 검색할 수 있습니다.
서지정보는 정규식 추출과 PubMed·Crossref 확인으로 보완하지만 부정확할 수 있습니다.
PDF 본문은 외부로 전송하지 않습니다. 기존 요약과 사용자 수정값은 보존하며, AI 번역
요청은 AI를 다시 켤 때까지 대기열에 유지합니다.
