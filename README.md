...
`요약 엔진 옵션` 두 카테고리로 나뉩니다. 엔진 옵션에서
제공자(Ollama/OpenAI/Anthropic)를 바로 바꾸고 모델을 관리합니다. 설치된 Ollama
모델은 설정 상단 목록에서 선택하는 즉시 활성 모델로 저장됩니다. 정리된
...
사용하지 않는 기여·한계 필드도 스키마에서 제외합니다. PaperPack과 파생 색인도
같은 필드명을 사용하며, 기존 파일의
...
`요약 엔진 옵션`의 `사양 다시 검사`는 별도 AI 런타임을 불러오지 않고 CPU·코어 수,
