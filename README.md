...
폴더와 스캔 주기·자동 보관 여부·주력 분야를 지정할 수 있습니다. 안정성이 두 번
확인된 PDF만 본문 지문을 만들고, ResearchGate 표지 변형을 포함한 중복 후보를
표시합니다. 확인된 중복의 앱 휴지통 이동은 항상 사용자가 승인해야 하며 휴지통
파일은 복원할 수 있습니다. 보관한 논문의 입력 PDF는 기본적으로 유지하고, 설정에서
...
시작할 때 `Paper Organizer 1.1.0`과 `Created by SANGKYU SHIN, Ph.D.`가 표시되는
...
결과는 `Output\PaperOrganizer_Setup_1.1.0.exe`입니다. 설치본에는 sPDF와 기본 OCR
