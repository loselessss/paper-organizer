...
4. AI 분석
   - Ollama 로컬 모델을 기본으로 사용하고, OpenAI·Anthropic은 사용자가
     전송에 동의한 경우에만 사용합니다.
...
- Ollama 모델은 설치본에 포함하지 않고, 사용자가 선택해 별도로 설치합니다.
...
설치본에는 앱, sPDF 연동 코드, 기본 OCR 런타임이 포함됩니다. Ollama LLM 가중치는
포함되지 않습니다.
...
sPDF는 `vendor/spdf` submodule로 고정된 버전을 사용하며, Paper Organizer는 sPDF의
업데이트 알림이나 자체 업데이트 기능을 사용하지 않습니다.
