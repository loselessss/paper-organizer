...
`vendor/spdf`는 검증된 sPDF 커밋을 고정하는 submodule입니다. 포함된 sPDF 버전은
서브모듈이 체크아웃한 태그/커밋과 `pdfeditor/meta.py`를 기준으로 확인합니다.
GUI 브리지는 sPDF를 지연 로딩하므로 PyQt5가 없는 코어 테스트 환경에서도 패키지를
가져올 수 있습니다.
...
설정과 오른쪽의 모델 선택·Ollama 설치/삭제·상주 설정·사양 추천을 나란히 보여줍니다.
...
있습니다. 라이브러리 목록에는 PaperPack이 처음 등록된 시간과 마지막 성공 분석일이
로컬 시간으로 표시됩니다. 상세 패널의 `AI 번역`은 현재 분석을 한국어로 번역해
원문/번역 보기를 토글하며, 원본 색인 필드는 덮어쓰지 않습니다. 분석 내용과 일치하는
번역은 PaperPack에 별도로 보관해 다시 호출하지 않습니다. 요약 요청에는 불필요한
thinking/reasoning을 사용하지 않습니다. Ollama는 지원되는 GPU를 자동 활용하고,
상주 옵션은 `자동`, `사용 후 즉시 해제`, `5분`, `30분`, `계속 상주` 중에서
고를 수 있습니다. 모델은 앱 시작과 동시에 적재하지 않고 첫 요청 뒤부터 유지하며,
상주 대상으로 고른 모델 하나 외의 모델은 요청 후 즉시 해제합니다. 자동은 저장된
PC 사양을 기준으로 보수적인 유지 시간을 정하고, 즉시 해제를 선택한 경우에만 앱이
직접 시작한 Ollama 서버도 작업 완료 후 종료합니다.
...
한국어 질문은 한국어 핵심어와 영문 원문 표현을 모두 검색하며, `A549`, `DMEM`,
유전자·단백질명, 시약명과 모델 번호 같은 식별자는 번역하지 않고 그대로 보존합니다.
검색어 계획과 근거 답변에는 실제 설치된 Ollama 모델을 우선 사용합니다. 현재 선택
모델이 설치돼 있으면 그대로 쓰고, 아니면 설치된 1.5~4B 호환 모델 중 가벼운 모델을
고릅니다. 로컬 호환 모델이 없을 때만 선택된 OpenAI·Anthropic 제공자로 넘어갑니다.
...
중단하며, `PDF 편집 취소`는 작업 복사본만 지웁니다. 적용 뒤 파일 정체성과 분석 큐를
...
이어서 다운로드 폴더·스캔 주기·자동 보관 방식을 정하는 `요약 감시 옵션`과
AI 제공자·모델을 정하는 `요약 엔진 옵션`을 차례로 표시합니다. 클라우드 제공자를
따로 선택하지 않았다면 Ollama 런타임과 요약 모델을 준비해야 분석을 시작할 수
있습니다.
...
선택 화면에는 모델 크기뿐 아니라 권장 용도·환각 위험·요약 전략과 최신 실논문
점수도 표시됩니다. 백그라운드 서지·Abstract는 Qwen3 1.7B, 정확도 우선 수동
본문 분석은 Qwen3.5 4B, 더 빠른 수동 대안은 Granite 4.1 3B로 안내합니다.
8B는 기여·한계를 포함한 정밀 요약용입니다. 8B 미만 본문 분석 모델은 구역별로
짧게 요약한 뒤 결과를 통합하며, 모든 등급에서 원문 확인이 필요합니다.
로컬 모델은 Intel Iris Xe 같은 내장 GPU Vulkan을 포함해 GPU를 먼저 사용하고,
사용할 수 없으면 CPU로 대체합니다. 내장 GPU에서는 1.7B 모델부터 시험하며 설정을
바꾼 뒤에는 Ollama를 완전히 재시작해야 합니다. 모델 설치 후 간이 검증은 실제
CPU·GPU 배치와 입출력 속도를 저장하고, 모델 실행 중 `사양 다시 검사`를 누르면
CPU·GPU·혼합 적재 상태를 다시 확인할 수 있습니다.
...
요약 모델을 바꿀 때는 실행 중인 Ollama 서버를 재시작하지 않습니다. 서버가 꺼져
있을 때만 `ollama serve`를 창 없이 시작하며, GPU 프로세스 설정을 적용하기 위해
재시작해야 할 때도 별도의 Ollama 데스크톱·트레이 앱을 열지 않습니다.
...
Ollama가 설치되어 있지만 실행 중이 아니면 같은 화면에 `Ollama 실행` 버튼이
나타나 서버만 띄웁니다. 설치되어 있지 않으면 앱 안에서 winget이나 App Installer를
호출하지 않고 공식 다운로드 주소만 복사·안내하며, 앱이 임의의 경로에서 파일을
내려받지 않습니다.
