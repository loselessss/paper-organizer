...
sidecar JSON과 통합 인덱스를 즉시 갱신합니다. 저널명 또는 학회명은
`bibliography.venue`에 별도 저장하며 라이브러리 목록·통합 검색·클라우드 편집본에
포함됩니다. 저널의 영향력이나 유명도는 근거 없이 추정하지 않습니다.

선택적으로 OneDrive 안의 폴더를 `JSON 미러`로 지정할 수 있습니다. 로컬 PDF 옆의
sidecar는 원본으로 유지하고, OneDrive에는 클라우드 도구가 수정하기 쉬운 단일
`portable-library.json`과 원본 JSON의 `backup/` 복사본을 서로 분리해 저장합니다.
...
클라우드 값을 적용하기 전 로컬 JSON은 편집 이력에 보관합니다. 클라우드에서
항목을 지워도 PDF와 로컬 원본 JSON은 자동 삭제하지 않습니다.

시작할 때 `Paper Organizer 0.3.1`과 `Created by SANGKYU SHIN, Ph.D.`가 표시되는
