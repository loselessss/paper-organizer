...
한 번에 분석합니다. 영어 원문에서 제목이 한국어로 번역된 명백한 오류를 감지하면
최종 JSON 생성만 강화 프롬프트로 한 번 재시도합니다.
...
이유를 표시합니다. 추천 버튼을 누르면 추천 모델이 선택된 모델 관리 창으로 이어지고,
그곳에서 설치 또는 검증을 마친 모델만 활성 모델로 선택합니다. 다운로드와 삭제는
...
즉시 요약과 자연어 검색도 Ollama가 설치되어 있지만 꺼져 있으면 요청 직전에 서버를
다시 시작합니다.
