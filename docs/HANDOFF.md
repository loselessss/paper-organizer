...
기본 조합으로 사용합니다. 리뷰 논문은 모든 소형 모델에서 점수가 크게 낮았으므로
일반 연구논문 점수와 분리해 평가합니다.

### (2-1) 다음 초기화 후 최우선: 리뷰 논문 전용 프롬프트

**2026-08-01 구현 완료.** `research_paper`·`review_paper`·`patent`를 분석 전에 결정하고, 사용자 확정 유형은 `document.type` 잠금으로 보호한다. 기존 PaperPack은 자동 변경하지 않고 라이브러리에 재분류 후보만 표시한다. 리뷰 전용 프롬프트와 rubric, 익명 Qwen3/Qwen3.5 실논문 점수는 저장소에 반영했다.
