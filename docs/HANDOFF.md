...
- 대상 브랜치: `main`
- 상태: v1.6.0 이후 요약 설정 재구성·설치 모델 즉시 선택·GUI 단일 실행 구현
- 이번 회차 범위: `요약 감시 옵션`/`요약 엔진 옵션` 분리, 요약 timeout 기본
  900초 및 사용자 조정, 설치 Ollama 모델 즉시 적용, 레거시 즉시 요약 UI 제거,
  앱 1.6 엔진 변경점·요약 언어 노출, 트레이 중복 방지
