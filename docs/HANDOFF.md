...
- 대상 브랜치: `main`
- 상태: v1.8.1 문서 유형 확정·리뷰 전용 분석·sPDF 선택 DTO·배포 보안 구현 완료
...
Granite 4.1 3B는 비공개 실논문 4편을 원문 언어로 순차 분석한 결과 4편 모두
완료, 평균 15.6초, JSON 재시도 0회, 소형 모델 기준 품질 82.5/100이었습니다.
속도는 `NVIDIA GeForce RTX 3080 Ti`에서 측정한 값이며 다른 PC에서는 달라집니다.
카탈로그와 모델 관리 화면에서 종합 추천 1순위로 표시하며, 충분한 메모리가
있는 자동·속도·균형·품질 프로필에서도 우선합니다. 기여·한계는 다른 8B 미만
모델과 마찬가지로 평가와 출력에서 제외합니다.

같은 `NVIDIA GeForce RTX 3080 Ti` 조건의 Qwen3 4B는 4편 모두 완료,
평균 15.1초, JSON 재시도 0회,
품질 80.8/100이었습니다. 전체 내용은 Granite보다 조금 더 생략하지만 포함한
수치 결과와 서지정보가 정확하고 연구질문·방법을 간결하게 구조화하는 강점이
있어 종합 추천 2순위로 표시합니다.

회사 노트북의 같은 비공개 실논문 4편에서는 Qwen3 1.7B가 평균 27.81점,
연구논문 3편 평균 33.75점, 평균 97.13초를 기록했습니다. Granite 3.3 2B는
전체 평균 25.31점, 연구논문 평균 30.42점, 평균 183.33초였습니다. 따라서
백그라운드는 Qwen3 1.7B, 사용자가 직접 시작하는 수동 분석은 Qwen3 4B를
기본 조합으로 사용합니다. 현재 데스크탑에서는 수동 분석을 Qwen3.5 4B로 교체했습니다.
리뷰 프롬프트 v4 실측은 Qwen3 1.7B 51.67점·해당 리뷰 서지 50점·6.975초,
Qwen3.5 4B 61.67점·해당 리뷰 서지 100점·14.391초이며 두 모델 모두 형식·언어
재시도 0회입니다. 수동 QA에서도 근거 없는 체계적 검색법과 혼합 언어가 없습니다.
리뷰논문 점수는 일반 연구논문과 분리해 평가합니다.

### (2-1) 다음 초기화 후 최우선: 리뷰 논문 전용 프롬프트

**2026-08-01 구현·실논문 재검증 완료.** `research_paper`·`review_paper`·`patent`를 분석 전에 결정하고, 사용자 확정 유형은 `document.type` 잠금으로 보호한다. 기존 PaperPack은 자동 변경하지 않고 라이브러리에 재분류 후보만 표시한다. 리뷰 v4는 섹션 단계부터 구체 대상·메커니즘·전략·응용을 보존하고, 축소 스키마에서는 결론과 근거 공백을 `summary`에 유지한다. 명시되지 않은 systematic review 검색법은 제거하며 리뷰 유형을 QA용 methods에 명시한다.

리뷰 논문을 일반 연구논문 프롬프트로 요약하지 않도록 문서 유형을 먼저 판별하고
전용 프롬프트와 평가 기준을 만듭니다. 리뷰 프롬프트는 개별 실험의 방법·결과를
억지로 채우지 않고, 검토 범위, 분류 체계, 주요 합의와 상충되는 결과, 근거 수준,
연구 공백과 향후 과제를 구조화해야 합니다. 연구논문과 리뷰 논문의 벤치마크 평균도
각각 표시하고, 리뷰 전용 정답표를 만든 뒤에만 기본 프롬프트로 채택합니다.
...
```text
docs/HANDOFF.md의 "(2-1) 다음 초기화 후 최우선: 리뷰 논문 전용 프롬프트"를
진행해줘. 첫 페이지에서 서지정보와 함께 연구논문/리뷰논문/특허를 먼저 분류하고,
분류 결과에 맞는 전용 프롬프트 파이프라인을 실행하도록 해.
기존 라이브러리에서 논문이 특허로 잘못 색인된 항목은 자동 변경하지 말고
재분류 후보로 보여준 뒤 사용자 승인 후 재색인하도록 구현해.
```
