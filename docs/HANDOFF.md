...
`vendor/spdf/pdfeditor/meta.py`를 기준으로 확인합니다.
...
- 상단 분석 큐 옆 프로젝트 버튼은 목록 팝업을 열고 닫습니다. 프로젝트를 선택하면
  팝업이 닫히고 기존 논문 표를 필터링합니다.
  목록 위 생성·관리 버튼과 우클릭으로 이름·설명 수정 및 삭제가 가능합니다.
  논문 우클릭에서 여러 프로젝트에 추가하거나 현재 프로젝트에서 제외합니다.
  소속은 PaperPack의 `curation.project_ids`에 저장하고, 빈 프로젝트를 포함한
  이름·설명은 라이브러리 루트 `projects.json` 설정에 저장합니다. 이 파일은
  재생성 가능한 색인이 아니므로 라이브러리 백업에 포함해야 합니다.
  프로젝트 삭제는 설정만 제거하며 논문은 수정하지 않습니다. 알 수 없는 소속 ID는
  무시하고 새 프로젝트에는 항상 새 UUID를 부여합니다.
- AI 요약 후 프로젝트 이름·설명과 논문 제목·요약으로 자동 분류합니다.
  기존 소속은 제거하지 않고 일치하는 프로젝트만 추가합니다. 수동 추가·제외는
  `curation.project_overrides`에 저장하여 재분석 후에도 유지합니다.
  프로젝트가 없거나 서지정보 전용 모드이면 분류 요청을 생략합니다.
  분류 실패는 요약 저장을 막지 않으며 `analysis.project_classification`에 기록합니다.
